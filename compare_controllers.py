...
rl_ee_base_path = "rl/logs/rsl_rl/2DOF-Hover-IK/2026-01-06_20-12-30_simpler-rewards"
# rl_ee_base_path = "rl/logs/rsl_rl/2DOF-Trajectory/2026-01-07_19-20-45_pos-tuning"
# rl_com_base_path = "../rl/logs/rsl_rl/Hover_PaperModels/2025-01-13_10-33-26_new_model_default_params_anneal_50e6_COM/"
# gc_base_path = "rl/logs/baseline_2dof_quad_control_reward_tune_no_ff_hover_flatness"
...
gc_base_path = "rl/logs/baseline_2dof_quad_control_reward_tune_no_ff_traj_flatness/"

error_legend_elements = [ 
                    Line2D([0], [0], color=params["rl_ee_color"], label='RL'),
                    # Line2D([0], [0], color=params["rl_com_color"], label='RL-COM'),
                    Line2D([0], [0], color=params["gc_color"], label='Decoupled Controller'),
...
violin_legend_elements_COM_ablation = [
                    Patch(facecolor=params["violin_color_1"], edgecolor=params["violin_color_1"], fill=True, label='RL'),
                    Patch(facecolor=params["violin_color_2"], edgecolor=params["violin_color_2"], fill=True, label='Decoupled Controller'),
] 
...
def load_data():
    ...
    # gc_path = os.path.join(gc_base_path, "eval_full_states.pt")
    gc_path = os.path.join(gc_base_path, "eval_traj_track_50Hz_eval_full_states.pt")
    ...
    return rl_ee_data, gc_data

@torch.no_grad()
def plot_error_pos_yaw(axs=None):
    rl_ee_data, gc_data = load_data()
    ...
    sns.lineplot(x=x_axis, y=rl_ee_pos_quantiles[1], ax=ins_ax, label="RL", color=params["rl_ee_color"], legend=False)
    # sns.lineplot(x=x_axis, y=rl_com_pos_quantiles[1], ax=ins_ax, label="RL-COM", color=params["rl_com_color"], legend=False)
    sns.lineplot(x=x_axis, y=gc__pos_quantiles[1], ax=ins_ax, label="Decoupled Controller", color=params["gc_color"], legend=False)
    ...
    sns.lineplot(x=x_axis, y=rl_ee_yaw_quantiles[1], ax=axs[1], label="RL", color=params["rl_ee_color"], legend=False)
    # sns.lineplot(x=x_axis, y=rl_com_yaw_quantiles[1], ax=axs[1], label="RL-COM", color=params["rl_com_color"], legend=False)
    sns.lineplot(x=x_axis, y=gc_yaw_quantiles[1], ax=axs[1], label="Decoupled Controller", color=params["gc_color"], legend=False)
    ...
    #Inset axis for the orientation plot
    x1, x2, y1, y2 = 9.0, 10.0, 0.0, 0.3
    ins_ax = axs[1].inset_axes([1.0, 0.4, 0.6, 0.6], xlim=(x1, x2), ylim=(y1, y2))
    axs[1].indicate_inset_zoom(ins_ax, edgecolor="grey")
    sns.lineplot(x=x_axis, y=rl_ee_yaw_quantiles[1], ax=ins_ax, label="RL", color=params["rl_ee_color"], legend=False)
    # sns.lineplot(x=x_axis, y=rl_com_pos_quantiles[1], ax=ins_ax, label="RL-COM", color=params["rl_com_color"], legend=False)
    sns.lineplot(x=x_axis, y=gc_yaw_quantiles[1], ax=ins_ax, label="", color=params["gc_color"], legend=False)
    ins_ax.fill_between(x_axis, rl_ee_yaw_quantiles[0], rl_ee_yaw_quantiles[2], alpha=0.2, color=params["rl_ee_color"])
    # ins_ax.fill_between(x_axis, rl_com_pos_quantiles[0], rl_com_pos_quantiles[2], alpha=0.2, color=params["rl_com_color"])
    ins_ax.fill_between(x_axis, gc_yaw_quantiles[0], gc_yaw_quantiles[2], alpha=0.2, color=params["gc_color"])
    ins_ax.yaxis.label.set_visible(False)
    ins_ax.set_xlim([x1, x2])
    ins_ax.set_xticks(np.linspace(x1, x2, 2))
    ins_ax.set_xticklabels([np.round(x, 2) for x in np.linspace(x1, x2, 2)])
...
def plot_violin_rmse_settling_time(axs=None):
    ...
    print("RMSEs")
    print(rl_ee_pos_rmse.mean(), rl_ee_yaw_rmse.mean()*180.0/np.pi, gc_pos_rmse.mean(), gc_yaw_rmse.mean()*180.0/np.pi)
    
    final_rl_ee_pos_error = rl_ee_pos_error[:, -1]
    final_rl_ee_yaw_error = rl_ee_yaw_error[:, -1] * 180.0 / np.pi
    final_gc_pos_error = gc_pos_error[:, -1]
    final_gc_yaw_error = gc_yaw_error[:, -1] * 180.0 / np.pi

    final_rl_ee_pos_rmse = torch.sqrt(torch.mean(final_rl_ee_pos_error**2)).cpu()
    final_rl_ee_yaw_rmse = torch.sqrt(torch.mean(final_rl_ee_yaw_error**2)).cpu()
    final_gc_pos_rmse = torch.sqrt(torch.mean(final_gc_pos_error**2)).cpu()
    final_gc_yaw_rmse = torch.sqrt(torch.mean(final_gc_yaw_error**2)).cpu()

    print("RMSEs at the end:")
    print(final_rl_ee_pos_rmse, final_rl_ee_yaw_rmse, final_gc_pos_rmse, final_gc_yaw_rmse)
    ...
    data = pd.DataFrame({
        "Settling Time": torch.cat([rl_ee_pos_settling_time, rl_ee_yaw_settling_time, gc_pos_settling_time, gc_yaw_settling_time]),
        "RMSE": torch.cat([rl_ee_pos_rmse, rl_ee_yaw_rmse, gc_pos_rmse, gc_yaw_rmse]),
        "Type": (["Position"] * N + ["Orientation"] * N + ["Position"] * N + ["Orientation"] * N),
        "Method": (["RL"] * 2*N + ["DC"] * 2*N)
    })

    # combined_data = pd.DataFrame({
    #     "Data": torch.cat([rl_ee_pos_settling_time, rl_ee_yaw_settling_time, gc_pos_settling_time, gc_yaw_settling_time, rl_ee_pos_rmse, rl_ee_yaw_rmse, gc_pos_rmse, gc_yaw_rmse]),
    #     "Metric": (["Settling Time"] * 6*N + ["RMSE"] * 6*N),
    #     "Type": (6*(["Position"] * N + ["Orientation"] * N)),
    #     "Method": (2*(["RL"] * 2*N + ["Decoupled Controller"] * 2*N)),
    # })
